...
release=node-v22.11.0-wii-ppc32-jitless-r2
destination=${1:-/root/node22-jitless-r2}
...
"$node" --jitless -e 'const assert=require("node:assert"); const b=Buffer.from("abc"); assert.strictEqual(b.toString("hex"),"616263"); assert.strictEqual(b.toString("base64"),"YWJj"); console.log(process.version,process.arch,process.platform);'
